function [data, srate] = readEGI(filename, chans)
    %  readegi() reads .raw files produced from EGI
    %  >> inputs
    %     filename -> .raw file you want to import
    %     chans    -> [vector] witch channel indices you want to import

    % Header information
    fid      = fopen(filename,'rb','b'); % load .raw file
    head     = readegihdr(fid);          % header information
    srate    = head.samp_rate;           % sampling rate
    
    % Preallocate    
    data = zeros(head.nchan + head.eventtypes, head.samples, 'int16');    

    % channels to import
    if nargin < 2
        chans = 1:head.nchan + head.eventtypes; end

    % EEG data
    data = fread(fid, [head.nchan + head.eventtypes, head.samples], ['*' head.datatype]);
    if size(data, 1) < max(chans)
        fprintf('Specified channels do not exist, adding a 0 matrix.\n')
        data = [data; zeros(max(chans) - size(data, 1), head.samples, class(data))];
    end
    data = data(chans, :);
    fclose(fid);
...
%     end
        ...
        function head = readegihdr(fid, forceversion)
            ...
            switch(head.version)
                case {2,3}
                    head.datatype = 'integer*2';
                case {4,5}
                    head.datatype = 'float32';
                case {6,7}
                    head.datatype = 'float64';
                otherwise,
